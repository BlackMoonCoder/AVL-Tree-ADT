 ...
 *      </pre>
 *
 *      DO NOT REMOVE THIS NOTICE (GNU GPL V2): Contact Information:
 *      duncanw@lsu.edu Copyright (c) 2022 William E. Duncan
 *
...
public class AVLTree<E extends Comparable<E>> implements AVLTreeAPI<E> {
	...
	public ArrayList<E> getChildren(E entry) throws AVLTreeException {
		...
		while (current != null) {
			int compare = cmp.compare(entry, current.data);
	...
	public E getParent(E entry) throws AVLTreeException {
		...
		if (inTree(entry)) {
			while (temp != null) {
				int ret = cmp.compare(entry, temp.data);
				if (ret < 0) {
					parent = temp;
					temp = temp.left;
				} else if (ret > 0) {
		...
		throw new AVLTreeException("Entry is not in tree");
	}

	public int ancestors(E entry) throws AVLTreeException {
		// Implement this method
		if (inTree(entry) == false) {
			throw new AVLTreeException("Entry not in Tree.");
		}
		;
		if (root == null) {
	...
	public int descendants(E entry) throws AVLTreeException {
		...
		while (temp != null) {
			...
			if (ret < 0) {
				...
				return countDesc(temp.left) + countDesc(temp.right);
			}
		}
		throw new AVLTreeException("Entry not in Tree.");
	}
	...
	/**
	 * An auxiliary method that inserts a new node in the tree or updates a node if
	...
	/**
	 * Traverses this subtree preorder and apply this specified function to the
	 * entry in each node
	 * 
	...
	/**
	 * Traverses this subtree postorder and apply this specified function to the
	 * entry in each node
	 * 
	 * @param node the root of the subtree
	 * @param func the function to apply to the entry in each node
	...
	private int countDesc(Node node) {
		...
		if (node.left != null) {
			if (node.right != null) {
				return countDesc(node.left) + countDesc(node.right) + 2;
			} else {
				return countDesc(node.left) + 1;
			}

		} else if (node.right != null) {
			return countDesc(node.right);
		} else {
			return 0;
		}
	}
